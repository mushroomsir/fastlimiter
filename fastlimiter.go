...
func (l *FastLimiter) Get(id string, policy ...int) (result Result, err error) {
	...
	if length == 0 {
		...
		for i, val := range policy {
			if val <= 0 {
				return result, errors.New("fastlimiter: must be positive integer")
			}
			p[i] = policy[i]
		}
	}
...
func (l *FastLimiter) getResult(id string, policy ...int) (Result, error) {
	var result Result
...
func (l *FastLimiter) getLimit(key string, args ...int) (res *limiterCacheItem) {
